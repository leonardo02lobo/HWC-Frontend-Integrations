...
export default function ProfileEditModal() {
	...
	useEffect(() => {
		const onOpen = (event: Event) => {
			const detail = (event as CustomEvent<Partial<ProfileValues>>).detail;
			setValues({
				name: detail?.name ?? "",
		...
		window.addEventListener("profile:open-edit", onOpen);
		return () => window.removeEventListener("profile:open-edit", onOpen);
	}, []);
	...
	const handleSubmit = (event: React.FormEvent) => {
		...
		const trimmedGithub = values.github.trim().replace(/^@/, "");
	...
	const inputClass =
		"w-full rounded-lg border border-gray-800/60 bg-[rgba(11,16,32,0.6)] px-3.5 py-2.5 text-sm text-white outline-none transition duration-200 placeholder:text-white/25 focus:border-[rgba(255,77,0,0.4)] focus:ring-2 focus:ring-[rgba(255,77,0,0.6)] font-[JetBrains_Mono,monospace]";
		...
		>
			...
			>
				<header className="flex items-start justify-between gap-3 border-b border-gray-800/60 px-5 py-4 md:px-6">
					<h2 id="profile-edit-title" className="text-lg text-white font-[Zen_Dots,cursive] md:text-xl">
				...
				<div className="flex flex-1 flex-col gap-5 overflow-y-auto px-5 py-5 md:px-6">
					...
					<label className="flex flex-col gap-1.5">
						<span className="text-sm text-white font-[JetBrains_Mono,monospace]">
							Nombre <span className="text-[#FF4D00]">*</span>
						</span>
						<input
							type="text"
							required
					...
					<label className="flex flex-col gap-1.5">
						<span className="text-sm text-white font-[JetBrains_Mono,monospace]">Usuario</span>
						<div className="flex items-center gap-2 rounded-lg border border-gray-800/60 bg-[rgba(11,16,32,0.6)] px-3 py-2.5 transition duration-200 focus-within:border-[rgba(255,77,0,0.4)] focus-within:ring-2 focus-within:ring-[rgba(255,77,0,0.6)]">
							<span className="text-sm text-white/40 font-[JetBrains_Mono,monospace]">@</span>
							<input
								type="text"
					...
					<label className="flex flex-col gap-1.5">
						<span className="text-sm text-white font-[JetBrains_Mono,monospace]">
							Correo <span className="text-[#FF4D00]">*</span>
						</span>
						<input
							type="email"
							required
					...
					<label className="flex flex-col gap-1.5">
						<span className="text-sm text-white font-[JetBrains_Mono,monospace]">Lenguaje preferido</span>
						<input
							type="text"
					...
					<label className="flex flex-col gap-1.5">
						<span className="text-sm text-white font-[JetBrains_Mono,monospace]">GitHub</span>
						<div className="flex items-center gap-2 rounded-lg border border-gray-800/60 bg-[rgba(11,16,32,0.6)] px-3 py-2.5 transition duration-200 focus-within:border-[rgba(255,77,0,0.4)] focus-within:ring-2 focus-within:ring-[rgba(255,77,0,0.6)]">
